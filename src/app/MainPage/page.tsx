...
export default function HomePage() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
      {/* Header */}
      <header className={`sticky top-0 z-50 ${isDarkMode ? 'bg-gray-900/80' : 'bg-gray-100/80'} backdrop-blur-md`}>
      ...
      {/* Featured Products */}
      <section className={`py-16 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`}>
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8">Featured Products</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div key={item} className={`group rounded-lg overflow-hidden shadow-lg transition-all duration-300 ${isDarkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-50'}`}>
                <img
      ...
      {/* Footer */}
      <footer className={`py-8 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-800'}`}>
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-between">
            <div className="w-full md:w-1/4 mb-6 md:mb-0">
              <h3 className="text-xl font-bold mb-4">LuxeStore</h3>
              <p className={isDarkMode ? 'text-gray-400' : 'text-gray-300'}>Elevating your lifestyle with premium products.</p>
            </div>
            <div className="w-full md:w-1/4 mb-6 md:mb-0">
              <h4 className="text-lg font-semibold mb-4">Quick Links</h4>
              <ul className={isDarkMode ? 'text-gray-400' : 'text-gray-300'}>
            ...
            <div className="w-full md:w-1/4">
              ...
              <form className="flex">
                <input
                  ...
                  className={`flex-grow px-4 py-2 rounded-l-full focus:outline-none ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-700 text-gray-100'}`}
                />
                <Button
                  type="submit"
          ...
          <div className="mt-8 text-center">
            <p className={isDarkMode ? 'text-gray-400' : 'text-gray-300'}>&copy; 2024 LuxeStore. All rights reserved.</p>
          </div>
        </div>
      </footer>
